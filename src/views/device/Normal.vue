          ...
          <a-row :gutter="24">
            ...
            <a-col :sm="12" :md="6" :lg="5">
              <a-form-item label="在线状态">
                <a-select placeholder="请选择状态" v-model="queryParam.deviceStatus" allowClear>
                  <a-select-option value="-1">全部</a-select-option>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList (arg) {
        this.loading = true
        postAction(`/consumer/doorLockSys/queryDoorLockDevice`, {
        'isBind': this.queryParam.isBind * 1,
        'deviceStatus': this.queryParam.deviceStatus * 1,
      ...
      .then((response) => {
        if (response.data.status === 200) {
          ...
          //   if (tar.indexOf(response.data.data[index].deviceName) === -1) {
          //     tar.push(response.data.data[index].deviceName)
          //   }
          // }
          // this.orderList = tar
        } else {
          this.$message.error('门锁列表查询失败')
        }
      })
      .catch(() => {
        this.$message.error('门锁列表查询失败')
